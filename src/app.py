...
# ============================================================================
st.markdown(
    "<h1 style='text-align: center;'>🌞 Solar PV Digital Twin Dashboard</h1>",
    unsafe_allow_html=True,
)
st.markdown(
    "<p style='text-align: center; color: rgba(255,255,255,0.7); font-size: 1.1rem;'>Real-time power prediction with ML-powered simulation</p>",
    unsafe_allow_html=True,
)
...
with tab1:
    ...
    st.markdown(
        "Enter environmental parameters and get instant power output predictions from the deployed model."
    )
    ...
    if st.button("🚀 Predict Power Output", use_container_width=True):
        ...
        try:
            ...
            st.error(
                "❌ **Connection Error**: Could not connect to API. Make sure the server is running:\n```\nuvicorn src.server:app --reload\n```"
            )
        except requests.exceptions.Timeout:
            st.error(
                "❌ **Timeout Error**: API request took too long. Check server health."
            )
...
with tab2:
    ...
    st.markdown(
        "Understand which environmental factors have the most impact on power output predictions."
    )
    ...
    if metrics and "feature_importance" in metrics:
        ...
        chart = (
            ...
            .properties(height=400, title="Feature Importance (RandomForest)")
            .configure_axis(labelColor="white", titleColor="white")
            .configure_title(color="#FFD700", fontSize=20)
...
with tab3:
    ...
    st.markdown(
        "Watch real-time simulated sensor data and power predictions streaming live."
    )
...
with tab4:
    ...
    st.markdown(
        "Upload a CSV file with multiple samples for bulk predictions via the API."
    )
