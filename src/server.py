...
class PredictionRequest(BaseModel):
    ...
    Solar_Irradiance_kWh_m2: float = Field(
        ..., description="Solar irradiance in kWh/m²", example=0.75
    )
    Temperature_C: float = Field(
        ..., description="Ambient temperature in Celsius", example=25.5
    )
    ...
    Panel_Tilt_deg: float = Field(
        ..., description="Panel tilt angle in degrees", example=30.0
    )
    Panel_Azimuth_deg: float = Field(
        ..., description="Panel azimuth angle in degrees", example=180.0
    )
    Plane_of_Array_Irradiance: float = Field(
        ..., description="POA irradiance", example=800.0
    )
    Cell_Temperature_C: float = Field(
        ..., description="Cell temperature in Celsius", example=35.0
    )
...
class BatchPredictionResponse(BaseModel):
    ...
    sample_predictions: List[Dict] = Field(
        ..., description="First 5 predictions as sample"
    )
    model_version: str = Field(..., description="Model version identifier")
    timestamp: str = Field(..., description="Prediction timestamp")
...
async def predict_batch(
    ...
    if not file.filename.endswith(".csv"):
        raise HTTPException(
            status_code=400, detail="File must be a CSV. Please upload a .csv file."
        )
