...
# ============================================================================
st.markdown(
    "<h1 style='text-align: center;'>🌞 Solar PV Digital Twin - Enhanced Dashboard</h1>",
    unsafe_allow_html=True,
)
st.markdown(
    "<p style='text-align: center; color: rgba(255,255,255,0.7); font-size: 1.1rem;'>Complete ML Pipeline with EDA, Advanced Metrics & Visualization</p>",
    unsafe_allow_html=True,
)
    ...
    if os.path.exists(viz_dir):
        ...
        with col1:
            if os.path.exists(f"{viz_dir}/01_target_distribution.png"):
                ...
                st.caption(
                    "Power Output Distribution: Shows the distribution and outliers in target variable"
                )
        ...
        with col1:
            if os.path.exists(f"{viz_dir}/03_correlation_heatmap.png"):
                ...
                if metrics and "correlation_with_target" in metrics:
                    ...
                    corr_sorted = sorted(
                        metrics["correlation_with_target"].items(),
        ...
        if os.path.exists(f"{viz_dir}/04_pairplot_top_features.png"):
            ...
            st.caption(
                "Pairplot: Pairwise relationships between most important features and target"
            )
        ...
        with col1:
            if os.path.exists(f"{viz_dir}/07_predicted_vs_actual.png"):
                ...
                st.caption(
                    "Predicted vs Actual: How well predictions match reality (closer to red line = better)"
                )
        ...
        with col2:
            if os.path.exists(f"{viz_dir}/06_residual_analysis.png"):
                ...
                st.caption(
                    "Residual Analysis: Distribution of prediction errors (centered at 0 = unbiased)"
                )
        ...
        with col1:
            if os.path.exists(f"{viz_dir}/08_error_distribution.png"):
                ...
                st.caption(
                    "Error Distribution: Absolute and percentage errors across predictions"
                )
        ...
        if os.path.exists(f"{viz_dir}/10_learning_curve.png"):
            ...
            st.caption(
                "Learning Curve: Model performance vs training set size (converging lines = well-trained)"
            )
        ...
        st.warning(
            "⚠️ Visualizations not found. Please run the enhanced training script first:"
        )
...
with tab2:
    ...
    st.markdown(
        "Enter environmental parameters and get instant power output predictions."
    )
    ...
    with col1:
        ...
        temp_c = st.number_input(
            "Temperature (°C)", min_value=-20.0, max_value=50.0, value=25.0, step=0.5
        )
        wind_speed = st.number_input(
            "Wind Speed (m/s)", min_value=0.0, max_value=20.0, value=3.0, step=0.1
        )
    ...
    with col2:
        ...
        panel_tilt = st.number_input(
            "Panel Tilt (degrees)", min_value=0.0, max_value=90.0, value=30.0, step=1.0
        )
        ...
        poa_irr = st.number_input(
            "POA Irradiance", min_value=0.0, max_value=1500.0, value=800.0, step=10.0
        )
        cell_temp = st.number_input(
            "Cell Temperature (°C)",
            min_value=-20.0,
            max_value=80.0,
            value=35.0,
...
with tab3:
    ...
    if metrics:
        ...
        if r2_gap < 0.05:
            st.success(
                f"✅ **Excellent Generalization**: Training R² ({train_r2:.4f}) and Test R² ({test_r2:.4f}) are very close (gap: {r2_gap:.4f}). Model generalizes well to unseen data!"
            )
        elif r2_gap < 0.10:
            st.info(
                f"ℹ️ **Good Generalization**: Small gap between Training R² ({train_r2:.4f}) and Test R² ({test_r2:.4f}). Gap: {r2_gap:.4f}. Acceptable performance."
            )
        else:
            st.warning(
                f"⚠️ **Potential Overfitting**: Significant gap between Training R² ({train_r2:.4f}) and Test R² ({test_r2:.4f}). Gap: {r2_gap:.4f}. Consider regularization or more data."
            )
...
with tab4:
    ...
    if model_artifact:
        ...
        with col1:
            if st.button(
                "▶️ Start" if not st.session_state.simulation_running else "⏸️ Stop"
            ):
            ...
            for i in range(50):
                ...
                if len(st.session_state.simulation_data) > 0:
                    ...
                    chart_placeholder.altair_chart(
                        power_chart, use_container_width=True
                    )

                    current_power = st.session_state.simulation_data[-1]["power"]
                    avg_power = np.mean(
                        [d["power"] for d in st.session_state.simulation_data]
                    )
                    ...
                    col1.metric(
                        ...
                        f"{current_power - avg_power:.2f} W",
                    )
        ...
        if st.button("🚀 Run Batch Prediction", use_container_width=True):
            try:
                with st.spinner("🔄 Sending to API..."):
                    ...
                    col1.metric(
                        "Mean Power", f"{result_df['Predicted_Power_W'].mean():.2f} W"
                    )
                    col2.metric(
                        "Max Power", f"{result_df['Predicted_Power_W'].max():.2f} W"
                    )
                    col3.metric(
                        "Min Power", f"{result_df['Predicted_Power_W'].min():.2f} W"
                    )
                    col4.metric(
                        "Std Dev", f"{result_df['Predicted_Power_W'].std():.2f} W"
                    )
...
with tab6:
    ...
    if metrics:
            ...
            params_df = pd.DataFrame(
                {
                    "Parameter": [
                        k.replace("model__", "")
                        for k in metrics["best_hyperparameters"].keys()
                    ],
        ...
        if "correlation_with_target" in metrics:
            ...
            ax.set_title(
                "Feature Correlations with Power Output", fontsize=14, fontweight="bold"
            )
        ...
        if "outlier_counts" in metrics:
            ...
            outlier_df["Percentage"] = (
                outlier_df["Outliers"] / total_samples * 100
            ).round(2)
            ...
            if total_outliers == 0:
                ...
                st.info(
                    f"ℹ️ Total outliers detected: {total_outliers} across all features"
                )
